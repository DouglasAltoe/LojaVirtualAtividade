...
async def excluir_usuario(inputDto: IdUsuarioDto):
    if UsuarioRepo.excluir(inputDto.id): return None
    pd = ProblemDetailsDto("int", f"O usuário com id <b>{inputDto.id}</b> não foi encontrado.", "value_not_found", ["body", "id_usuario"])
